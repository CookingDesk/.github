<div align="center">

<img src="../assets/logo.svg" alt="CookingDesk Logo" width="180"/>

# CookingDesk

</div>

<div align="center">

❗ **In development**

</div>

---

## 🌍 About the App

**CookingDesk** is an application designed to help you explore, save, and cook recipes from all around the world. Whether you’re a beginner or an experienced cook, CookingDesk guides you step-by-step through every recipe, making the cooking experience simple and enjoyable.

You can also create your own recipes and store them online using a JSON-based storage system powered by :contentReference[oaicite:0]{index=0}.

---

## 🍲 Features

- 🔎 Search recipes from different cuisines worldwide  
- 📌 Save your favorite recipes for quick access  
- 👨‍🍳 Step-by-step cooking instructions (with videos too) 
- 🧑‍🍳 Create and upload your own recipes  
- ☁️ Store recipes remotely using JSON storage  
- 📖 Clean and easy-to-follow cooking interface  

---

## 🚀 Developer

Developed by **[elvarHex](https://github.com/elvarHex)**  
:contentReference[oaicite:1]{index=1}

---

## 📌 Notes

CookingDesk is currently in active development. Features may change, improve, or expand over time.

---

## 🗓️ Release date (unconfirmed)

September 2026

---
